...
class TestDeleteTweets(unittest.TestCase):
    def test_tweet_reader_retweet(self):
        ...
        for idx, val in enumerate(TweetReader(FakeReader(tweets), restrict="retweet")):
    ...
    def test_tweet_reader_reply(self):
        ...
        for idx, val in enumerate(TweetReader(FakeReader(tweets), restrict="reply")):
            self.assertEqual(expected[idx]['tweet_id'], val['tweet_id'])
    ...
    def test_tweet_reader_date(self):
        ...
        for idx, val in enumerate(TweetReader(FakeReader(tweets), date=date(2014, 2, 1))):
            self.assertEqual(expected[idx]['tweet_id'], val['tweet_id'])
